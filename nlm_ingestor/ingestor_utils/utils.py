...
def sent_tokenize(org_texts):
    ...
    for org_text in org_texts.split("\n"):
        ...
        modified_text = re.sub(
            r"^([.,?!]\s+)+", "", org_text
        )  # To handle bug https://github.com/nltk/nltk/issues/2925
...
def blocks_to_sents(blocks, flatten_merged_table=False, debug=False):
    ...
    for block_idx, block in enumerate(blocks):
        ...
        if block_type == "header":
            ...
            if (
                prev_header
                and block["level"] <= prev_header["level"]
                and len(levels) > 0
            ):
            ...
            if debug:
                print(
                    "-",
                    [
                        str(level["level"]) + "-" + level["block_text"]
                        for level in levels
                    ],
                )
        ...
        for level in levels:
            level_chain.append(
                {"block_idx": level["block_idx"], "block_text": level["block_text"]}
            )
        ...
        if (
            ...
            and "is_table_start" not in block
        ):
